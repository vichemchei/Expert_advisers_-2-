...
# ============================================================================
# SESSION CONFIGURATION (ADJUSTED FOR GMT+3 BROKER TIME - NAIROBI)
# ============================================================================

# IMPORTANT: Your broker time is GMT+3 (EAT - East Africa Time)
# When broker shows 7:00 AM, actual GMT is 4:00 AM
# So we ADD 3 hours to GMT times to get broker time

SESSION_CONFIG = {
    'ASIAN': {
        'symbols': [
            'USDJPY#',   # Most active
            'AUDJPY#',   # Best for Asian hours
            'NZDJPY#',   # NZ opens first
            'AUDUSD#',   # Sydney/Tokyo
            'NZDUSD#',   # Wellington opens first
        ],
        'hours': [(3, 12)],  # Broker: 06:00 - 15:00 (GMT+3)
        'risk_multiplier': 0.85,
        'max_spread_pips': 2.5,
    },
    'LONDON': {
        'symbols': [
            'EURUSD#',   # Highest volume
            'GBPUSD#',   # London native
            'EURGBP#',   # Pure European
            'EURJPY#',   # Crosses active
            'GBPJPY#',   # High volatility
        ],
        'hours': [(10, 19)],  # Broker: 13:00 - 22:00 (GMT+3)
        'risk_multiplier': 1.0,
        'max_spread_pips': 2.0,
    },
    'NEW_YORK': {
        'symbols': [
            'EURUSD#',   # Peak liquidity
            'GBPUSD#',   # NY/London overlap
            'USDCAD#',   # North American
            'USDJPY#',   # Still active
        ],
        'hours': [(15, 24), (0, 3)],  # Broker: 18:00 - 03:00 (GMT+3)
        'risk_multiplier': 1.0,
        'max_spread_pips': 2.0,
    },
}

# Correlation groups to avoid simultaneous trading
CORRELATION_GROUPS = {
...
class SessionManager:
            ...
            for start_hour, end_hour in config['hours']:
                if start_hour <= current_hour < end_hour:
                    if self.current_session != session_name:
                        ...
                        self.logger.info(f"  GMT Time: {now.strftime('%H:%M')}")
                        self.logger.info("="*60)
                    return session_name
        
        if self.current_session is not None:
            self.logger.info(f"{Fore.YELLOW}[NO ACTIVE SESSION] Current GMT: {current_hour}:00{Style.RESET_ALL}")
            self.current_session = None
    ...
    def get_active_symbols(self) -> List[str]:
        ...
        symbols = SESSION_CONFIG[session]['symbols']
        return symbols
...
class FVGDetector:
    ...
    def detect_fvgs(self, df: pd.DataFrame, symbol: str, timeframe: str) -> List[FVG]:
        """Detect Fair Value Gaps in recent data with proper validation"""
        new_fvgs = []
        atr = self.calculate_atr(df)
        
        # Increase minimum gap size to 0.2 * ATR (20% of ATR)
        min_gap_size = atr * 0.2  
        
        # Scan last 100 candles instead of 50 for better coverage
        lookback_start = max(2, len(df) - 100)
        ...
        for i in range(lookback_start, len(df) - 1):
            ...
            # PROPER Bullish FVG Detection
            # Gap between candle 1 high and candle 3 low
            # Candle 2 should not fill this gap (strong momentum)
            if candle1['high'] < candle3['low']:
                gap_size = candle3['low'] - candle1['high']
                
                # Validate gap size and momentum candle
                if gap_size >= min_gap_size:
                    # Check that candle 2 created strong upward momentum
                    ...
                    if candle2_is_bullish and candle2_body > (atr * 0.1):
                        ...
                        self.logger.info(
                            ...
                            f"{fvg.bottom:.5f}-{fvg.top:.5f} (Gap: {gap_size*10000:.1f} pips){Style.RESET_ALL}"
                        )
            
            # PROPER Bearish FVG Detection
            # Gap between candle 3 high and candle 1 low
            if candle1['low'] > candle3['high']:
                gap_size = candle1['low'] - candle3['high']
                
                # Validate gap size and momentum candle
                if gap_size >= min_gap_size:
                    # Check that candle 2 created strong downward momentum
                    candle2_body = abs(candle2['close'] - candle2['open'])
...
class SessionBasedFVGBOSBot:
    ...
    def check_entry_signal(self, symbol: str, trade_tf: str, trend_tf: str) -> Optional[Dict]:
        ...
        if bos and bos.direction == trend_direction:
            ...
            if nearest_fvg:
                # Check if price is in FVG zone (50-70% fill)
                if bos.direction == 'bullish':
                    fvg_range = nearest_fvg.top - nearest_fvg.bottom
                    entry_zone_bottom = nearest_fvg.bottom
                    entry_zone_top = nearest_fvg.bottom + (0.7 * fvg_range)
                    ...
                    fvg_range = nearest_fvg.top - nearest_fvg.bottom
                    entry_zone_top = nearest_fvg.top
                    entry_zone_bottom = nearest_fvg.top - (0.7 * fvg_range)
    ...
    def print_status(self):
        ...
        gmt_time = datetime.utcnow().strftime('%H:%M:%S')
        ...
        print(f"{'='*80}")
        print(f"Time - GMT: {gmt_time} | Local: {local_time}")
