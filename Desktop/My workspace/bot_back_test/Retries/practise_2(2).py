...
def update_loss_streak(pnl):
    """Update loss streaks separately for normal and reverse bias"""
    global loss_streak, reverse_bias
    if pnl < 0:  # trade lost
        loss_streak += 1
        print(f"Loss streak: {loss_streak}")
        if not reverse_bias and loss_streak >= normal_loss_limit:
            reverse_bias = True
            loss_streak = 0
            print("Bias switched to REVERSE mode")
        elif reverse_bias and loss_streak >= reverse_loss_limit:
            reverse_bias = False
            loss_streak = 0
            print("Bias switched back to NORMAL mode")
    else:  # trade win or breakeven
        loss_streak = 0
    ...
    try:
        while True:
            ...
            for symbol in symbols:
                ...
                # Apply reverse bias if active
                if reverse_bias and signal:
                    signal = "sell" if signal == "buy" else "buy"
                    print(f"  {symbol}: Reverse bias active -> flipping signal to {signal.upper()}")
                ...
                if signal:
                    ...
                    if result:
                        ...
                        # Monitor trade closure
                        time.sleep(2)
                        closed = False
                        while not closed:
                            positions = get_open_positions(symbol)
                            if not positions:  # position closed
                                history = mt5.history_deals_get(datetime.now() - pd.Timedelta(days=1), datetime.now())
                                if history:
                                    last_deal = history[-1]
