...
risk_per_trade = 0.02  # Changed from 0.20 (20%) to 0.02 (2%)
stop_loss_pips = 80
...
def calculate_lot_size(balance, risk_pct, sl_pips, symbol_info):
    """
    Fixed lot size calculation for gold
    """
    # Get actual contract size and tick value from MT5
    contract_size = symbol_info.trade_contract_size
    tick_value = symbol_info.trade_tick_value
    tick_size = symbol_info.trade_tick_size
    
    # Calculate pip value
    pip_value = tick_value * (0.1 / tick_size)  # For gold, 0.1 is typically 1 pip
        ...
        if in_position:
            if position_type == 'buy':
                if current_high >= take_profit:
                    # Correct P&L calculation for XAUUSD
                    pips_gained = (take_profit - entry_price) / point
                    pip_value_per_lot = 10  # $10 per pip for 0.01 lot
                    pnl = pips_gained * pip_value_per_lot * lot_size_used
                    balance += pnl
                    trades.append({
                        'symbol': symbol,
                        'type': 'buy',
                    ...
                    pips_gained = (stop_loss - entry_price) / point
                ...
                if current_low <= take_profit:
                    pips_gained = (entry_price - take_profit) / point
                    pip_value_per_lot = 10
                    pnl = pips_gained * pip_value_per_lot * lot_size_used
                    balance += pnl
                    trades.append({
                    ...
                    pips_gained = (entry_price - stop_loss) / point
                    pip_value_per_lot = 10
                    pnl = pips_gained * pip_value_per_lot * lot_size_used
                    balance += pnl
                    trades.append({
...
def plot_results(results, metrics):
    ...
    fig.suptitle('RSI-Stochastic-EMA Bot Backtest Results (H1 Timeframe)', 
    ...
    if results[0] and results[0]['balance_history']:
        ...
        axes[0, 0].set_xlabel('Time Period (H1 bars)')
    ...
    plt.tight_layout()
    plt.savefig('backtest_rsi_stoch_ema_fixed.png', dpi=300, bbox_inches='tight')
    print("\nChart saved as 'backtest_rsi_stoch_ema_fixed.png'")
    plt.show()


def run_backtest():
    print("=" * 70)
    print("RSI-STOCHASTIC-EMA BOT BACKTEST (FIXED)")
    print("=" * 70)
    ...
    print(f"\nStrategy Logic:")
    print(f"  BUY: Stochastic %K < 20 AND RSI > 50 AND Price > EMA200")
    print(f"  SELL: Stochastic %K > 80 AND RSI < 50 AND Price < EMA200")
    print(f"\nIndicators:")
    ...
    if metrics:
        ...
        if trades_df is not None and not trades_df.empty:
            trades_df.to_csv('backtest_rsi_stoch_ema_trades_fixed.csv', index=False)
            print(f"\nDetailed trade log saved as 'backtest_rsi_stoch_ema_trades_fixed.csv'")
