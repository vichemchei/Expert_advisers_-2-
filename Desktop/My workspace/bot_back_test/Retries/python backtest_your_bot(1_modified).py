...
# Trading Windows (EAT)
TRADING_WINDOWS = [
    (4, 0, 5, 0),      # 4:00 AM - 5:00 AM
    (10, 0, 11, 0),    # 10:00 AM - 11:00 AM
    (14, 30, 16, 0),   # 2:30 PM - 4:00 PM
    (16, 30, 18, 0),   # 4:30 PM - 6:00 PM
]
...
# --------- Helper Functions ---------
def is_within_trading_window(timestamp):
    """Check if timestamp is within trading windows (EAT)"""
    eat_tz = pytz.timezone('Africa/Nairobi')
    time_eat = timestamp.astimezone(eat_tz)
    
    hour = time_eat.hour
    minute = time_eat.minute
...
def calculate_lot_size(balance, risk_pct, sl_pips, symbol_info):
    """
    Calculate position size based on risk - FIXED VERSION
    """
...
def backtest_strategy(symbol, start_date, end_date):
    ...
    # Generate signals (only during trading windows)
    df['in_trading_window'] = df['time'].apply(is_within_trading_window)
    df['signal'] = 0
    
    # Buy signal: RSI > 50 AND price > MA AND within trading window
    df.loc[(df['rsi'] > 50) & (df['close'] > df['ma']) & df['in_trading_window'], 'signal'] = 1
    
    # Sell signal: RSI < 50 AND price < MA AND within trading window
    df.loc[(df['rsi'] < 50) & (df['close'] < df['ma']) & df['in_trading_window'], 'signal'] = -1
...
def run_backtest():
    ...
    print(f"\nTrading Windows (EAT):")
    for i, (sh, sm, eh, em) in enumerate(TRADING_WINDOWS, 1):
