"""
RSI-MA-ATR Trading Bot with ML integration for MT5 - DIAGNOSTIC VERSION
...
- Enhanced visibility into signal generation and ML filtering
...
def diagnose_no_signal(symbol, df):
    ...
    # Calculate ATR for bounce tolerance
    ...
    # SELL conditions with tolerance
...
def rule_signal_from_df(df, symbol):
    ...
    # Calculate ATR for flexible bounce detection
    atr_series = calc_atr(df, atr_period)
    atr_val = atr_series.iloc[-1] if atr_series is not None and not pd.isna(atr_series.iloc[-1]) else 0
    bounce_tolerance = 0.5 * atr_val if atr_val > 0 else 20  # Allow 0.5 ATR tolerance or 20 points
    
    # BUY signal: price was near or below EMA_short, now above
    buy_bounce = prev['low'] <= (prev['ema_short'] + bounce_tolerance)
    ...
    # SELL signal: price was near or above EMA_short, now below
    sell_bounce = prev['high'] >= (prev['ema_short'] - bounce_tolerance)
...
def ml_filtered_signal(symbol, clf, scaler, df):
    ...
    if rule == 'buy':
            ...
            #logger.info(f"[APPROVED] {symbol}: ML-FILTERED BUY SIGNAL APPROVED (prob={prob_buy:.3f} >= {ML_PROB_THRESHOLD})")
            return 'buy'
        #else:
            #logger.warning(f"[REJECTED] {symbol}: BUY signal REJECTED by ML filter (prob={prob_buy:.3f} < {ML_PROB_THRESHOLD})")
            #return None
...
def run_live(symbols_list, mode='live'):
    """Main execution loop with diagnostic capabilities"""
    if mode == 'collect':
    ...
    logger.info('='*60)
    logger.info('STARTING DIAGNOSTIC LIVE LOOP')
    logger.info('='*60)
    ...
    logger.info(f"Models loaded per symbol:")
    for symbol in symbols_list:
        clf, reg_lot, reg_trail, scaler = models_cache[symbol]
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='ML-Enhanced Trading Bot - DIAGNOSTIC VERSION')
    ...
    logger.info("=" * 60)
    logger.info("RSI-MA-ATR Trading Bot - DIAGNOSTIC VERSION")
    logger.info("Enhanced logging to identify execution issues")
    logger.info("=" * 60)
