...
def main():
    ...
    symbol_configs = {
        ...
        'GBPUSD#': {
            'swing_lookback': 11,      # More volatile, larger lookback
            'min_bos_distance': 5,
            'min_fvg_atr': 0.06,       # Slightly larger FVG filter
            'risk_reward_ratio': 2.0,
            'cooldown_minutes': 30,
        },
        'USDJPY#': {
            'swing_lookback': 13,      # Choppier, needs more confirmation
            'min_bos_distance': 6,
            'min_fvg_atr': 0.05,
            'risk_reward_ratio': 2.0,
            'cooldown_minutes': 30,
        ...
        'AUDUSD#': {
            'swing_lookback': 12,      # Exotic, more noise
            'min_bos_distance': 6,
            'min_fvg_atr': 0.06,
            'risk_reward_ratio': 2.0,
            'cooldown_minutes': 35,
    ...
    config = {
        ...
        'max_open_trades': 20,
        'max_exposure': 2.0,
        'max_daily_loss_pct': 50.0,      # FIXED from 50%!
        'max_daily_loss_usd': 50.0,
        
        # Trailing stop settings
        'initial_stop_distance_atr': 1.0,  # Initial SL distance in ATR
        'trailing_stop_activation': 1.0,   # Profit needed to activate trailing (in R multiples)
        'trailing_stop_distance': 0.5,     # Trailing distance in ATR
